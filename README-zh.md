...
本项目是由 [Websoft9](https://www.websoft9.com) 研发的 **云原生程序** ，大大简化 Alfresco 复杂的安装及配置。

## 系统要求

安装本项目，确保符合如下的条件（[详细参考](https://docs.alfresco.com/content-services/latest/install/containers/docker-compose)）：
...
```
sudo wget -N https://raw.githubusercontent.com/Websoft9/StackHub/main/docker-installer.sh; sudo bash docker-installer.sh -r alfresco
...
```
sudo wget -N https://raw.githubusercontent.com/Websoft9/StackHub/main/docker-installer.sh; sudo bash docker-installer.sh -r alfresco -p
```
2.复制package包后安装

将压缩的package包复制到想要安装的服务器或虚拟机，登录 Linux，运行下面的**自动化命令**即可启动安装并显示安装结果。
```
sudo bash install-alfresco
```
...
| 名称 | 端口号 | 用途 |  必要性 |
| --- | --- | --- | --- |
| alfresco | 8080 | 浏览器访问 Alfresco | Y |
| postgres | 5432 | TCP 访问Postgres数据库 | Y |
## 文档

[Alfresco 管理员手册](https://support.websoft9.com/docs/alfresco)

## 企业级支持

如果需要企业级支持，请订阅我们提供的 [Alfresco 企业级支持版](https://apps.websoft9.com/alfresco) 
